	...
	public class AstarPathEditor : Editor {
		...
		TextAsset SaveGraphData (byte[] bytes, TextAsset target = null) {
			...
			if (target != null) {
				...
				int i = 0;
				do {
					path = $"Assets/NavMeshCaches/{SceneManager.GetActiveScene().name}.bytes";
					i++;
		...
		public static TextAsset SaveGraphData(byte[] bytes, string graphName, TextAsset target = null) 
		{
			string projectPath = System.IO.Path.GetDirectoryName(Application.dataPath) + "/";
			...
			{
				path = $"Assets/NavMeshCaches/{graphName}.bytes";
			}
